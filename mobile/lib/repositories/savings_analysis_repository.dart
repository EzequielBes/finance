...
class SuggestedCut {
  SuggestedCut({
    ...
    required this.cutPercentOfCategory,
  });
  final Category category;
...
class SavingsAnalysisRepository {
  ...
  List<SuggestedCut> suggestCuts(
    List<SavingsCandidate> allCandidates,
    double targetPercent,
  ) {
    final withHistory = allCandidates.where((c) => c.reference > 0).toList();
    if (withHistory.isEmpty) return [];

    final totalReference = allCandidates.fold<double>(
      0.0,
      (sum, c) => sum + c.currentMonthAmount,
    );
    final targetReais = targetPercent / 100 * totalReference;

    final weights = {
      for (final c in withHistory)
        c.category.id: c.currentMonthAmount / c.reference,
    };
    ...
    return withHistory.map((c) {
      ...
      final cutPercent = c.currentMonthAmount > 0
          ? cutAmount / c.currentMonthAmount * 100
          : 0.0;
      return SuggestedCut(
        category: c.category,
        cutAmount: cutAmount,
        cutPercentOfCategory: cutPercent,
      );
    }).toList();
  }
}
